...
% imgs_folder='C:\SCI_TMP\projects\whales\imgs\cutout_manually'; % perfect
% imgs_folder='C:\SCI_TMP\projects\whales\imgs\afewnotprocessed';
% imgs_folder='C:\SCI_TMP\projects\whales\imgs\allwhalesnobackground\BmPhotoSubset_ETP_18Jan2012-NOBACKGROUND'; % all whales, background manually deleted
% imgs_folder='C:\SCI_TMP\projects\whales\imgs\humpbacks';
% imgs_folder='C:\SCI_TMP\projects\whales\imgs\killerwhales';
% imgs_folder='C:\SCI_TMP\projects\whales\imgs\humpbacks2_clayoquotwhales.ca';
% imgs_folder='C:\SCI_TMP\projects\whales\imgs\WW_DanaPoint_120825';
% imgs_folder='C:\SCI_TMP\projects\whales\imgs\BmPhotoSubset_ETP_18Jan2012';
% imgs_folder='C:\SCI_TMP\projects\whales\imgs\BmPhotoSubset_ETP_18Jan2012_repeatedwhales\with_masks_20120826';
% imgs_folder='C:\SCI_TMP\projects\whales\imgs\BmPhotoSubset_ETP_18Jan2012_problem'; % two photos of the same whale
% imgs_folder='C:\SCI_TMP\projects\whales\imgs\BmPhotoSubset_ETP_18Jan2012--kNN_kNNa';
% imgs_folder='C:\SCI_TMP\projects\whales\imgs\WW_TA_augsep2012_goodquality';
% imgs_folder='C:\SCI_TMP\projects\whales\imgs\flickr_Ka_May2013--kNN_kNNa+kNN';
% imgs_folder='C:\SCI_TMP\projects\whales\imgs\flickr_Ka_May2013--kNN_kNNa';
% imgs_folder='C:\SCI_TMP\projects\whales\imgs\web_20130526';
% imgs_folder='C:\SCI_TMP\projects\whales\imgs\web20130526_test';
% imgs_folder='C:\SCI_TMP\projects\whales\imgs\NOAA-Paula_set2_20120907--kNN_kNNa+kNN_hori';
imgs_folder='C:\SCI_TMP\projects\whales\imgs\all_20130728';
...
mask_erosion_rad_percent=0.05; % for finding the radius of erosion adaptively, 0.1 (default) <-> 10%
...
% calculate and store SIFT features for all images
% DON'T FORGET TO UPDATE THIS SECTION IF MODIFYING IMAGE ANALYSIS BELOW
fprintf('Calculating SIFT features for all %u images ', Nimgs);
sift_features={};

% disp('DBG: #image1=60:end')	% for debugging
...
%     end
    for j=startj:Nimgs
        ...
        img2mask_name=strrep(img2mask_name,'.JPG','Mask.JPG');
...
%         [F2 D2] = vl_sift(J);
        ...
        %%
        F1im=F1(1:2,m1); % coordinates of centers of matched features of image 1
        F2im=F2(1:2,m2); % the same for image2

        
        %% >>> kNN reduction
        kNNred_inds=kNN_matches_reduction(F1im,F2im,kNN_k,KNN_RED_PARAM);
        %<<<
        ...
        %%
        F1im=F1(1:2,m1); % coordinates of centers of matched features of image1
        F2im=F2(1:2,m2); % the same for image2

        
        %% >>> kNNangles reduction
        tic;
        kNNangl_inds=kNNangles_matches_reduction(F1im,F2im,kNNangl_k,KNNANGL_RED_THRESH,KNN_RED_PARAM);
        ela_kNNangred=toc;
        %<<<
        
        
        %% features left after the kNNangles reduction
        ...
        %% FIGURE 1, MATCHING-REDUCTION PROCESS: plot the feature left after the kNNangles reduction
        if PLOT_FIGURE
            figure(1)
            subplot(211)
        ...
        %% >>> hori reduction (horizontal orientation)
        tic;
        hori_inds=horientation_matches_reduction(F1im_red,F2im_red);
        ela_horired=toc;
        %<<<
        
        %% features left after the hori reduction
        matches_inside_hori=matches_inside_kNNangl(:,hori_inds);
        matches_img1_hori=matches_img1_kNNangl(hori_inds,:);
        matches_img2_hori=matches_img2_kNNangl(hori_inds,:);
        
        fprintf('   %u matches skept after hori reduction\n', sum(~hori_inds))
        
        
        %% FIGURE 1, MATCHING-REDUCTION PROCESS: plot the feature left after the hori reduction
        if PLOT_FIGURE
            % super title for the whole plot
            suptitle(sprintf('%u > %u (kNN) > %u (kNNa) > %u (hori)', sum(bothinside_mask), sum(kNNred_inds), sum(kNNangl_inds), sum(hori_inds)));
        ...
        if PLOT_FIGURE
            ...
            if ~isempty(score)
                sscore=sort(score);
                str1=sprintf('%u matches[%.2f] found, shortest distances: ', size(matches_img1_hori,1),ubcthresh);
                suptitle(sprintf('%s (%.1fs)',str1,ela_siftI1+ela_siftI2+ela_kNNred+ela_kNNred));
            else
        ...
        RES_ubcmatch{i,j}={matches,score,bothinside_mask,matches_inside,matches_inside_kNN,matches_inside_kNNangl,matches_inside_hori};
        RES_sift{i,j}={F1,F2,D1,D2};
        RES_Nmatches(i,j)=length(matches_inside_hori);
        RES_Nmatches_inside(i,j)=length(matches_inside_hori);
        
        if i>10
            disp('');
        end
        
        fprintf('ELA: %.1fs %.1fs %.1fs %.1fs %.1fs', ela_siftI1, ela_siftI2, ela_ubcmatch, ela_kNNred, ela_kNNangred);
        fprintf('\n');
